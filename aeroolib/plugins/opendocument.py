...
def output_encode(iterator, method='xml', encoding=None):
    ...
    """
    return b''.join([bytes(i, encoding=encoding) for i in iterator])
...
class Template(MarkupTemplate):
    ...
    def __init__(
        self, source, serializer, filepath=None, styles=None,
    ...
    def _aeroo_statements(self, tree):
        ...
        s_xpath = (
            "//text:a[starts-with(@xlink:href, 'python://')]"
            "| //text:placeholder"
            "| //text:text-input[starts-with(@text:description, '<') and "
            "substring(@text:description, string-length(@text:description))='>']"
        )
    ...
    def _handle_aeroo_tags(self, tree):
        """Treat all aeroo tag (py:if/for/choose/when/otherwise) tags."""
        ...
        for r_node, parsed in r_statements:
            ...
            if directive is not None:
                ...
                # This is need for removing only closing genshi tag except
                # other children tags of <p> tag
                if (
                    len(outermost_c_ancestor) > 1 and
                    closing in outermost_c_ancestor.getchildren()
                ):
                    outermost_c_ancestor_copy = deepcopy(outermost_c_ancestor)
                    outermost_c_ancestor.remove(closing)
                    outermost_c_ancestor \
                        .getparent().append(outermost_c_ancestor_copy)
                    outermost_c_ancestor = outermost_c_ancestor_copy

                wrap_nodes_between(
                    outermost_o_ancestor, outermost_c_ancestor, genshi_node)
            else:
    ...
    def _handle_column_loops(self, statement, ancestor, opening,
        ...
        update_py_attrs(
            enclosed_cell, "__aeroo_inc_col_count(%d)" % loop_id)
        ...
        if repeat_node is not None:
            ...
            # definitions nodes
            attribs = {
                "opening": bytes(opening_pos, encoding='utf-8'),
                "closing": bytes(closing_pos, encoding='utf-8'),
                "table": table_name
            }
    ...
    def _handle_hyperlinks(self, tree):
        xpath_href_expr = (
            "//draw:a[starts-with(@xlink:href, 'python://')] | "
            "//text:a[starts-with(@xlink:href, 'pythonuri://')]"
        )
            ...
            for attrs in element.keys():
                if not attrs.startswith('{%s}' % GENSHI_URI):
                    element.attrib[attrs] = \
                        element.attrib[attrs].replace(PREFIX, PREFIX * 2)
            if element.text:
...
class Manifest(object):
    ...
    def _remove_thumbnails(self):
        """Remove thumbnails from the manifest.

        Thumbnails must be removed because these are pictures
        of the template instead of pictures of the rendered file.

        Thumbnails entries look like the following:
        <manifest:file-entry
            manifest:full-path="Thumbnails/thumbnail.png"
            manifest:media-type="image/png"/>
        """
        manifest = self.namespaces['manifest']

        file_entry_tag = "{%s}file-entry" % manifest
        file_entries = [
            c for c in self.root.iterchildren()
            if c.tag == file_entry_tag
        ]

        for el in file_entries:
            full_path_tag = '{%s}full-path' % manifest
            thumnails = [
                a for a in el.items()
                if a[0] == full_path_tag and
                a[1].startswith("Thumbnails")
            ]
            if thumnails:
                self.root.remove(el)
    ...
    def add_file_entry(self, path, mimetype=None):
        ...
        attribs = {
            '{%s}media-type' % manifest_namespace: mimetype or '',
            '{%s}full-path' % manifest_namespace: path
        }
        entry_node = EtreeElement(
            '{%s}%s' % (manifest_namespace, 'file-entry'),
            attrib=attribs, nsmap={'manifest': manifest_namespace})
...
class Meta(object):
    ...
    def add_property(self, data=None, ptype=''):
        meta_namespace = self.namespaces['meta']
        entry_node = EtreeElement(
            '{%s}%s' % (self.namespaces['meta'], 'user-defined'),
            nsmap={'meta': meta_namespace},
...
class OOSerializer:
    ...
    def check_tabs(self, tree, namespaces):
        tags = tree.xpath(
            "//text:*/text()[contains(string(), '%s')]/.." % TAB,
        ...
        for tag in tags:
            if tag.text:
                ...
                for text in tabs_text:
                    new_node = EtreeElement(
                        '{%s}tab' % namespaces['text'],
                        attrib={},
                        nsmap=namespaces)
                    new_node.tail=text
    ...
    def check_spaces(self, tree, namespaces):
        ...
        for tag in tags:
            if tag.text:
                ...
                new_node = EtreeElement(
                    '{%s}s' % namespaces['text'],
                    attrib=tag.attrib, nsmap=namespaces)
                tag.addnext(new_node)
            for child in tag.iterchildren():
                if not child.tail:
                    continue
    ...
    def check_new_lines(self, tree, namespaces):
        span_tags = tree.xpath(
            "//text:span[text()[contains(., '%s')]]" % NEW_LINE,
            namespaces=namespaces)
        tags = tree.xpath(
            "//text:*[text()[contains(., '%s')]]" % NEW_LINE,
            namespaces=namespaces)
        ...
        for tag in tags:
            ...
            if tag.text:
                ...
                new_node = EtreeElement(
                    '%s' % tag.tag, attrib=tag.attrib, nsmap=namespaces)
                last_node = new_node
                tag.addnext(new_node)
            if children:
                for child in children:
                    if tag.text and tag.text.endswith(NEW_LINE):
                        new_node = EtreeElement(
                            tag.tag, attrib=tag.attrib, nsmap=namespaces)
        ...
        for tag in span_tags:
            span_tag = '{%s}span' % namespaces['text']
            ...
            if tag.tag == span_tag:
                ...
                new_parent_node = EtreeElement(
                    '%s' % parent.tag, attrib=parent_attrib, nsmap=namespaces)
                ...
                try:
                    while(True):
                        ...
                        if (
                            curr_child.tag == span_tag and
                            tag.text == curr_child.text
                        ):
                            new_span_node = EtreeElement(
                                span_tag, attrib=tag.attrib, nsmap=namespaces)
                            ...
                            # set tail of source node in tail of latest new node
                            new_span_node.tail = tag.tail
                            break
                        else:
                ...
                try:
                    while(True):
                        ...
                        if not next_child.text:
                            ...
                            new_node = EtreeElement(
                                '%s' % parent.tag,
                                attrib=parent_attrib, nsmap=namespaces)
    ...
    def check_images(self, tree, namespaces):
        ...
        for tag in tags:
            ...
            tag.getparent().attrib.update({
                '{%s}height' % namespaces['svg']: height,
    ...
    def apply_style(self, oo_styles):
        if oo_styles:
            ...
            self.styles_new_xml = bytes(self.styles_new)
            self.styles = TStyle(self.inzip.read(STYLES))

            # font-face
            font_face_new = self.styles_new.root.xpath(
                '//office:font-face-decls/style:font-face',
                namespaces=self.styles_new.namespaces)
            font_face_orig = self.styles.root.xpath(
                '//office:font-face-decls/style:font-face',
                namespaces=self.styles.namespaces)
            self._replace_style_by_attrib(
                font_face_new, font_face_orig, 'name',
                self.styles.namespaces['style'])

            # styles
            new_styles = self.styles_new.root.xpath(
                '//office:styles/style:style',
                namespaces=self.styles_new.namespaces)
            orig_styles = self.styles.root.xpath(
                '//office:styles/style:style',
                namespaces=self.styles.namespaces)
            self._replace_style_by_attrib(
                new_styles, orig_styles, 'name',
                self.styles.namespaces['style'])

            # master-styles
            new_master_page_styles = self.styles_new.root.xpath(
                '//office:master-styles/style:master-page',
                namespaces=self.styles_new.namespaces)
            orig_master_page_styles = self.styles.root.xpath(
                '//office:master-styles/style:master-page',
                namespaces=self.styles.namespaces)
            self._replace_style_by_attrib(
                new_master_page_styles, orig_master_page_styles,
                'name', self.styles.namespaces['style'])

            # automatic-styles
            new_automatic_page_styles = self.styles_new.root.xpath(
                '//office:automatic-styles/style:style',
                namespaces=self.styles_new.namespaces)
            orig_automatic_page_styles = self.styles.root.xpath(
                '//office:automatic-styles/style:style',
                namespaces=self.styles.namespaces)
            if orig_automatic_page_styles:
                self._replace_style_by_attrib(
                    new_automatic_page_styles,
                    orig_automatic_page_styles, 'name',
                    self.styles.namespaces['style'])
            else:
            ...
            new_page_layout = self.styles_new.root.xpath(
                '//office:automatic-styles/style:page-layout',
                namespaces=self.styles_new.namespaces)
            orig_page_layout = self.styles.root.xpath(
                '//office:automatic-styles/style:page-layout',
                namespaces=self.styles.namespaces)
            self._replace_style_by_attrib(
                new_page_layout, orig_page_layout,
                'name', self.styles.namespaces['style'])
            ###########################
            self.styles_xml = bytes(self.styles)
    ...
    def _replace_style_by_attrib(self, node_list1, node_list2, name, namespace):
        name_tag = '{%s}%s' % (namespace, name)

        for node1 in node_list1:
            curr_node = None
            node_to_update = []
            for node2 in node_list2:
                if node2.get(name_tag) == node1.get(name_tag):
                    curr_node = node2
